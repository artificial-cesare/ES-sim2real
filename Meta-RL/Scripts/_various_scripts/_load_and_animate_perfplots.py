...
ADAPT = True  # if False, the script will only show perf of the policy without adaptation
...
perf_adap = pd.DataFrame(columns=["mass", "friction", "reward"])
...
for mass, friction in combinations:
    random_task = {'mass':mass, 'friction':friction}
    ...
    mean_reward, std_reward = evaluate_policy(model_results, env, n_eval_episodes=5, deterministic=True)
    print('Before adaptation: ', mass, 'kg \t', friction, 'friction \t', 'Train levels: ', mean_reward, '+-', std_reward)
    ...
    if ADAPT:
        ...
        mean_reward, std_reward = evaluate_policy(model_results, env, n_eval_episodes=5, deterministic=True)
        print('After adaptation: ', mass, 'kg \t', friction, 'friction \t', 'Train levels: ', mean_reward, '+-', std_reward)

        y = pd.DataFrame({'mass': [mass], 'friction': [friction], 'reward': [mean_reward]})
        perf_adap = pd.concat([perf_adap, y], ignore_index=True)

global_min = np.min(perf["reward"])
global_max = np.max(perf["reward"])

pivot_table = perf.pivot('friction', 'mass', 'reward')

fig = plt.figure( figsize=(15, 5))  # Adjusted for dynamic number of classes
sns.heatmap(pivot_table, annot=False, vmin=global_min, vmax=global_max, cmap='viridis')
fig.suptitle('Agent Performance before adaptation')
plt.show()

if ADAPT:
    global_min = np.min(perf_adap["reward"])
    global_max = np.max(perf_adap["reward"])

    pivot_table = perf_adap.pivot('friction', 'mass', 'reward')

    fig = plt.figure( figsize=(15, 5))  # Adjusted for dynamic number of classes
    sns.heatmap(pivot_table, annot=False, vmin=global_min, vmax=global_max, cmap='viridis')
    fig.suptitle('Agent Performance after adaptation')
    plt.show()
