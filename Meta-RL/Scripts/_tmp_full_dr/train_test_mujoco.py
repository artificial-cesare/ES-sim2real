...
def make_env(rank, seed, env_name, dr_specs_dict, wrapper_class):
        def _init():
            env = gym.make(env_name)#, healthy_reward=0, reset_noise_scale=5e-2)
            env = wrapper_class(env, randomization_specs=dr_specs_dict)
...
def train_model(env_name, rand_class, seed, train_steps, 
                ...
                sample_task=None, sde_sample_freq=4):
    """
    ...
    if rand_class == "SetParams":
        ...
        env = make_vec_env(env_name, n_envs=n_envs, seed=seed, 
                           vec_env_cls=DummyVecEnv, wrapper_class=wrapper_class,
                            wrapper_kwargs={'randomization_specs': dr_specs_dict}) 
    ...
    ## DEFINE MODEL
    if rand_class == "Rand" or rand_class == "SetParams": 
        ...
        model = RecurrentPPO("MlpLstmPolicy",
                ...
                use_sde=True, seed=seed, device=device)
        print(model.policy)
...
def test_model(model_path, env_path, env_name, rand_class, dr_specs_dict, seed, n_envs, dr_specs, n_eval_episodes=50):
    """
    ...
    wrapper_class = DR_Wrapper
    env = make_vec_env(env_name, n_envs, seed=seed, vec_env_cls=DummyVecEnv, wrapper_class = wrapper_class, wrapper_kwargs={'randomization_specs': dr_specs})
    env = VecNormalize.load(env_path, env)
    env.training = False
    env.norm_reward = False

    # set params for testing 
    if rand_class == "SetParams" or rand_class == "Rand":
