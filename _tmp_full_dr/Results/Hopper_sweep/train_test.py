...
def train_model(env_name, seed, train_steps, policy_kwargs, n_envs, n_steps, batch_size, n_epochs, gamma, ent_coef, gae_lambda, config):
    ...
    model = PPO("MlpPolicy", env, verbose=1, policy_kwargs=policy_kwargs, n_steps=n_steps, batch_size=batch_size, n_epochs=n_epochs, gamma=gamma, ent_coef=ent_coef, gae_lambda=gae_lambda, tensorboard_log="tensorboard/", use_sde=use_sde, sde_sample_freq=4,seed=seed, device=device)
    model.learn(total_timesteps=train_steps, tb_log_name=f'{env_name}')

    model_path = folder_path+f'{env_name}_{seed}_{config}.ckpt'
    env_path = folder_path+f'{env_name}_vecnormalize_{seed}_{config}.pkl'
...
def test_model(model_path, env_path, env_name, seed, n_envs, config, n_eval_episodes=50):
    """
    ...
    perf_path = folder_path+f'perf_{env_name}_{seed}_{config}.csv'
